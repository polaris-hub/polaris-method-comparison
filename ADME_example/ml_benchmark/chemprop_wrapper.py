...
class ChemPropWrapper:
    ...
    def validate(self, train, test):
        pred = run_chemprop(train, test, self.y_name, num_epochs=50, accelerator="gpu")
        return pred
...
class ChemPropRDKitWrapper:
    ...
    def validate(self, train, test):
        pred = run_chemprop_rdkit(train, test, self.y_name, num_epochs=50, accelerator="gpu")
        return pred
...
def run_chemprop(train, test, y_col, num_epochs=30, accelerator="gpu"):
    ...
    # mpnn = models.MPNN.load_from_checkpoint("converted_model_st.pt")
    # mpnn.message_passing.W_i = torch.nn.Linear(in_features=ffn_input_dim, out_features=300, bias=False)
    # mpnn.message_passing.W_i = torch.nn.Linear(in_features=86, out_features=300, bias=False)
    # Train the model
    with warnings.catch_warnings():
        warnings.filterwarnings('ignore')
        with DisableLogger():
    ...
    pred_tensor = torch.concat(predictions, axis=1).mean(axis=1, keepdim=True)
...
# def run_chemprop_rdkit(train, test, y_col, num_epochs=20, accelerator="gpu"):
#     # generate the validation set
#     train, val = train_test_split(train, test_size=len(test))
